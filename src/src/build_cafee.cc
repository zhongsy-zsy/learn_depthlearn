...
int main() {
  ...
  auto caffer_parser = nvcaffeparser1::createCaffeParser();

  char* txt = "c.prototxt";
  char* model_name = "c.caffemodel";

  auto blob_name_to_tensor =
      caffer_parser->parse(txt, model_name, *network, DataType::kFLOAT);
  ...
  //          giemodelstream->size());
  serialize_output_stream.open("./caffe.trt", std::ios::binary | std::ios::out);
